 ...
 * 授权控制器
 *
 * @author xujiping 2018-01-18 17:01
 */
@Controller
public class AuthorizeController {
    ...
    public Object authorize(Model model, HttpServletRequest request) throws OAuthSystemException,
        URISyntaxException {
        ...
        try {
            ...
            //检查传入的客户端id是否正确
            clientId = authAuthzRequest.getClientId();
            if (!oAuthService.checkClientId(clientId)) {
                OAuthResponse response = OAuthASResponse.errorResponse(HttpServletResponse
                    .SC_BAD_REQUEST).setError(OAuthError.TokenResponse.INVALID_CLIENT)
            ...
            //得到客户端重定向地址
            String redirectURI = authAuthzRequest.getParam(OAuth.OAUTH_REDIRECT_URI);
            //构建响应
            final OAuthResponse response = builder.location
            ...
            // 返回错误消息（如?error=）
            final OAuthResponse response = OAuthASResponse.errorResponse(HttpServletResponse
                .SC_FOUND)
