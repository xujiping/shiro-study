 ...
 * 访问令牌控制器
 *
 * @author xujiping 2018-01-19 11:17
 */
@RestController
public class AccessTokenController {
    ...
    public HttpEntity token(HttpServletRequest request) throws OAuthSystemException {
        try {
            ...
            //检查传入的客户端id是否正确
            if (!oAuthService.checkClientId(authAuthzRequest.getClientId())) {
            ...
            //检查客户端安全KEY是否正确
            if (!oAuthService.checkClientSecret(authAuthzRequest.getClientSecret())) {
            ...
            String authCode = authAuthzRequest.getParam(OAuth.OAUTH_CODE);
            // 检查验证类型，此处只检查AUTHORIZATION_CODE类型，其他的还有PASSWORD或REFRESH_TOKEN
            if (authAuthzRequest.getParam(OAuth.OAUTH_GRANT_TYPE).equals(GrantType
